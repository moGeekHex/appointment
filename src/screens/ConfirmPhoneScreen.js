...
class ConfirmPhoneScreen extends Component {
    ...
    getCode = async () =>
    {
        ...
        const url = 'https://us-central1-appointment-832ca.cloudfunctions.net/genCode';
        const profile = this.props.navigation.getParam('profile');
        const phone = this.state.value;
        
        try{
            const { data }= await axios.post(url,{ phone : phone , uid : profile.uid });
            if(data.success)
            {
    ...
    verifyCode = async () =>
    {
        ...
        const url = 'https://us-central1-appointment-832ca.cloudfunctions.net/verifyCode';
        const profile = navigation.getParam('profile');
        const code = parseInt(this.state.value , 10);

        try{
            const { data } = await axios.post( url , { code : code , uid : profile.uid });
            if(data.success)
            {
                this.setState({ loading : false , value : '' , verifying : true });
    ...
    render()
    {
        ...
        const btnTxt = verifying ? 'Enter Number' : 'Get Verification Code';
